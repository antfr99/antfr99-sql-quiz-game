...
if scenario.startswith("15"):
    ...
    def merge_sources(imdb, my, votes):
        ...
        if imdb.empty or my.empty:
            return pd.DataFrame()
        # Only keep expected columns if they exist
        keep_cols = [c for c in ['Movie ID', 'Title', 'IMDb Rating'] if c in imdb.columns]
        merged = my.merge(imdb[keep_cols], on='Movie ID', how='left')
    ...
    # --- Query Router with Fuzzy Matching + Column Checks ---
    def query_router(query, df):
        ...
        # Highest IMDb
        if match(["highest imdb", "top imdb", "best imdb"]) and 'IMDb Rating' in df.columns:
            row = df.loc[df['IMDb Rating'].idxmax()]
            return f"The highest IMDb-rated film is '{row.get('Title','Unknown')}' with a rating of {row['IMDb Rating']}."

        # Lowest IMDb
        if match(["lowest imdb", "worst imdb"]) and 'IMDb Rating' in df.columns:
            row = df.loc[df['IMDb Rating'].idxmin()]
            return f"The lowest IMDb-rated film is '{row.get('Title','Unknown')}' with a rating of {row['IMDb Rating']}."

        # Highest personal rating
        if match(["highest my rating", "top-rated", "best my rating"]) and 'Your Rating' in df.columns:
        ...
        if match(["lowest my rating", "worst my rating"]) and 'Your Rating' in df.columns:
        ...
        if match(["average", "mean rating"]):
            avg_my = round(df['Your Rating'].mean(), 1) if 'Your Rating' in df.columns else "N/A"
            avg_imdb = round(df['IMDb Rating'].mean(), 1) if 'IMDb Rating' in df.columns else "N/A"
            return f"Your average rating is {avg_my}, and the average IMDb rating is {avg_imdb}."

        # Most voted
        if match(["most voted", "popular"]) and 'Num Votes' in df.columns:
            row = df.loc[df['Num Votes'].idxmax()]
            return f"The most voted film is '{row.get('Title','Unknown')}' with {row['Num Votes']} votes."
